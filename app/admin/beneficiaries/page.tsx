...
function AdminStatusBadge({ status }: { status: ApplicationStatus }) {
  switch (status) {
    ...
    case "Released":
      return <Badge variant="default" className="bg-emerald-500">Released</Badge>;
  }
}
    ...
    <div className="min-h-screen bg-background flex flex-col">
      <header className="sticky top-0 z-50 border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
        <div className="mx-auto flex h-14 w-full items-center justify-between px-6">
          <div className="flex items-center gap-3">
            ...
            <span className="text-sm font-medium">Admin: Application Management</span>
          </div>
          <Link href="/dashboard">
      ...
      <main className="flex-1 p-6 mx-auto w-full max-w-7xl">
        <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h1 className="text-2xl font-bold tracking-tight">Beneficiary Applications</h1>
            <p className="text-sm text-muted-foreground">Review registrations, update status, and record benefit releases.</p>
          </div>
          <Input
        ...
        <Card>
          <CardContent className="p-0">
            <Table>
              ...
              <TableBody>
                {filtered.map((b) => {
                  const mainProgram = b.programs[0]?.program || "None";
                  ...
                  return (
                    <React.Fragment key={b.id}>
                      <TableRow className={isActionRow ? "bg-muted/30" : ""}>
                        ...
                        <TableCell className="text-xs text-muted-foreground">{b.municipality}, {b.province}</TableCell>
                        <TableCell><Badge variant="secondary" className="text-[10px]">{mainProgram}</Badge></TableCell>
                        <TableCell className="text-xs text-muted-foreground">{b.dateRegistered}</TableCell>
                        <TableCell><AdminStatusBadge status={b.applicationStatus} /></TableCell>
                        <TableCell className="text-right space-x-2">
                          ...
                          {(b.applicationStatus === "Approved" || b.applicationStatus === "Released") && (
                            <Button
                              variant="secondary"
                              size="sm"
                              className="h-7 text-xs"
                      ...
                      {isActionRow && actionType === "status" && (
                        <TableRow className="bg-muted/30 hover:bg-muted/30">
                          <TableCell colSpan={7} className="p-4">
                            <form onSubmit={handleStatusChange} className="flex flex-col sm:flex-row items-end gap-4 bg-background p-4 rounded-lg border">
                              <Field className="w-48">
                                <FieldLabel>New Status</FieldLabel>
                                <Select value={newStatus} onValueChange={(v: ApplicationStatus) => setNewStatus(v)}>
                                  <SelectTrigger>
                                    <SelectValue />
                                  </SelectTrigger>
                                  <SelectContent>
                              ...
                              <div className="flex gap-2 shrink-0">
                                <Button type="button" variant="ghost" onClick={cancelAction}>Cancel</Button>
                      ...
                      {isActionRow && actionType === "release" && (
                        <TableRow className="bg-muted/30 hover:bg-muted/30">
                          <TableCell colSpan={7} className="p-4">
                            <form onSubmit={handleRecordRelease} className="flex flex-col sm:flex-row items-end gap-4 bg-background p-4 rounded-lg border border-emerald-500/30">
                              ...
                              <div className="flex gap-2 shrink-0 ml-auto">
                                <Button type="button" variant="ghost" onClick={cancelAction}>Cancel</Button>
                                <Button type="submit" className="bg-emerald-600 hover:bg-emerald-700">Confirm Release</Button>
                              </div>
                            </form>
                          </TableCell>
                        </TableRow>
                ...
                {filtered.length === 0 && (
                  <TableRow>
                    <TableCell colSpan={7} className="h-24 text-center">No applications found.</TableCell>
                  </TableRow>
                )}
              </TableBody>
            </Table>
