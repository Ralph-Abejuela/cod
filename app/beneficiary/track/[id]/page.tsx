    ...
    <div className="min-h-screen bg-background print:bg-white print:min-h-0">
      ...
      <main className="mx-auto max-w-5xl px-6 py-8 print:p-0">
              ...
              <CardContent>
                <div className="relative border-l pl-6 ml-2 space-y-8">
                  ...
                  <div className="relative">
                    ...
                    <p className="text-xs text-muted-foreground">{beneficiary.dateRegistered}</p>
                  </div>
                  ...
                  {beneficiary.applicationStatus === "Rejected" ? (
                    <div className="relative">
                      ...
                      <p className="text-xs text-muted-foreground">{beneficiary.dateRejected}</p>
                    ...
                    <div className="relative">
                      <div className={`absolute -left-[31px] top-1 h-3 w-3 rounded-full ${beneficiary.dateApproved ? 'bg-primary' : 'bg-muted border-2 border-primary/50'}`} />
                      <p className={`text-sm font-medium ${!beneficiary.dateApproved && 'text-muted-foreground'}`}>
                        Application Approved
                      </p>
                      {beneficiary.dateApproved && (
                        <>
                          <p className="text-xs text-muted-foreground">{beneficiary.dateApproved}</p>
                          <p className="text-xs text-muted-foreground mt-0.5">Approved by: {beneficiary.approvedBy}</p>
                        </>
                      )}
                    </div>
                  )}
                  ...
                  {beneficiary.applicationStatus !== "Rejected" && (
                    <div className="relative">
                      <div className={`absolute -left-[31px] top-1 h-3 w-3 rounded-full ${beneficiary.dateReleased ? 'bg-emerald-500' : 'bg-muted border-2 border-primary/50'}`} />
                      <p className={`text-sm font-medium ${!beneficiary.dateReleased && 'text-muted-foreground'}`}>
                        Benefits Released
                      </p>
                      {beneficiary.dateReleased && (
                        <p className="text-xs text-muted-foreground">{beneficiary.dateReleased}</p>
                      )}
                    </div>
                  )}
                </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <Card size="sm">
                  <CardHeader>
                    ...
                    <CardTitle className="text-2xl text-emerald-600">₱{totalReceived.toLocaleString()}</CardTitle>
                  </CardHeader>
                </Card>
                <Card size="sm">
                  <CardHeader>
        ...
        <div className="space-y-4">
          <h2 className="text-lg font-semibold tracking-tight">Benefits Received</h2>
          <Card>
              ...
              {releases.length > 0 ? (
                <Table>
                  ...
                  <TableBody>
                    {releases.map((r) => (
                      <TableRow key={r.id}>
                        <TableCell className="whitespace-nowrap">{r.dateReleased}</TableCell>
                        <TableCell className="font-medium">{r.assistanceType}</TableCell>
                        <TableCell>
                        ...
                        <TableCell className="text-xs">{r.releasingOfficer}</TableCell>
